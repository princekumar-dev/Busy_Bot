...
export function EvoQRConnector() {
    ...
    const BOT_NAME = import.meta.env.VITE_EVO_API_BOT_NAME || "busybot";
    ...
    const fetchQR = async () => {
        ...
        try {
            // First try to check instance status
            const statusRes = await fetch(`${baseUrl}/instance/status?key=${API_KEY}&id=${BOT_NAME}`);

            if (statusRes.ok) {
                const statusData = await statusRes.json();
                // Assume API structure based on standard wa-baileys wrappers (like Evolution API)
                if (statusData?.instance?.status === "open") {
                    setStatus("connected");
                    setLoading(false);
                    return;
                }
            ...
            // If not connected, init/create instance and fetch QR
            const createRes = await fetch(`${baseUrl}/instance/create`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ instanceName: BOT_NAME, token: API_KEY }),
            });

            // Based on evolution API, we must connect after creation
            const connectRes = await fetch(`${baseUrl}/instance/connect/${BOT_NAME}`, {
                method: "GET", // Sometimes it's GET in Evolution API
                headers: {
                    "apikey": API_KEY,
            ...
            if (!connectRes.ok) {
                throw new Error(`Failed to connect: ${connectRes.statusText}`);
            }

            const data = await connectRes.json();

            // Evolution API typically returns base64 or raw pairing code. We'll look for standard fields
            if (data?.base64) {
                // If it's a base64 image (data:image/png;base64,...), we just set it as an image source later (or parse the code if we want to use qrcode.react)
                // we'll assume qrcode data string is returned for qrcode.react
                setQrCode(data.base64); // Need to adapt based on actual API response, Evolution usually sends `base64`
            } else if (data?.code) {
                setQrCode(data.code);
            } else if (data?.qrcode) {
